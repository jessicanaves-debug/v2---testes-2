...
// ─── Theme patching ──────────────────────────────────────────────────────────
// pptxgenjs não permite customizar as cores do tema (theme1.xml).
// Após gerar o pptx como blob, abrimos o ZIP e substituímos o theme1.xml
// com o esquema de cores + fontes idêntico ao modelo base Branddi.

const BRANDDI_THEME_XML = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="Branddi">
  <a:themeElements>
    <a:clrScheme name="Branddi">
      <a:dk1><a:srgbClr val="000000"/></a:dk1>
      <a:lt1><a:srgbClr val="FCFCFC"/></a:lt1>
      <a:dk2><a:srgbClr val="595959"/></a:dk2>
      <a:lt2><a:srgbClr val="EEEEEE"/></a:lt2>
      <a:accent1><a:srgbClr val="EA9999"/></a:accent1>
      <a:accent2><a:srgbClr val="0296A6"/></a:accent2>
      <a:accent3><a:srgbClr val="0ACFDE"/></a:accent3>
      <a:accent4><a:srgbClr val="FF6B65"/></a:accent4>
      <a:accent5><a:srgbClr val="4BBECD"/></a:accent5>
      <a:accent6><a:srgbClr val="A6CBC2"/></a:accent6>
      <a:hlink><a:srgbClr val="0097A7"/></a:hlink>
      <a:folHlink><a:srgbClr val="0097A7"/></a:folHlink>
    </a:clrScheme>
    <a:fontScheme name="Branddi">
      <a:majorFont>
        <a:latin typeface="Archivo SemiBold"/>
        <a:ea typeface=""/>
        <a:cs typeface=""/>
      </a:majorFont>
      <a:minorFont>
...
async function patchTheme(pptxBlob: Blob): Promise<Blob> {
  const zip = await JSZip.loadAsync(pptxBlob);
  // Substitui o theme1.xml com o tema Branddi
  zip.file("ppt/theme/theme1.xml", BRANDDI_THEME_XML);
...
const COLORS = {
  primary: "0296A6",           // Títulos de slide (ciano/teal do modelo base)
  primaryDark: "000000",       // Fallback background escuro (dk1)
  cyan: "0296A6",              // Números grandes, destaques (accent2)
  cyanLight: "4BBECD",         // Bordas leves, detalhes (accent5)
  cyanDark: "0296A6",          // Variante escura do ciano
  white: "FFFFFF",
  textDark: "000000",          // Corpo de tabelas, textos escuros (dk1)
  textMuted: "595959",         // Labels, subtítulos, notas (dk2)
  border: "E5E7EB",
  bgLight: "F8FAFB",
  success: "10B981",
  danger: "EF4444",
  warning: "F59E0B",
  blueAccent: "177A8B",        // Detalhes em azul nos gráficos (sidenotes)
  boxDark: "595959",           // Boxes escuros (ROI, Saving) — dk2 no modelo base
};

const FONT_HEADING = "Archivo SemiBold";
...
function addSlideChrome(slide: pptxgen.Slide, logo: string | null, bg: string | null) {
  ...
  if (logo) {
    slide.addImage({ data: logo, x: 0.4, y: 0.3, w: 0.35, h: 0.35 });
  }
  slide.addText("branddi", {
    x: 0.8, y: 0.32, w: 1.2, h: 0.35,
    fontFace: FONT_HEADING, fontSize: 12, color: COLORS.primary,
  });
}
...
function addContentTitle(slide: pptxgen.Slide, title: string, periodo?: string) {
  slide.addText(title, {
    ...
    fontFace: FONT_HEADING, fontSize: 18, color: COLORS.primary,
  });
  if (periodo) {
    slide.addText([
      { text: "Período: ", options: { bold: true, fontSize: 11, fontFace: FONT_HEADING, color: COLORS.textDark } },
      { text: periodo, options: { fontSize: 11, fontFace: FONT_BODY, color: COLORS.textDark } },
    ], {
...
function addCoverSlide(pptx: pptxgen, data: PresentationData, coverBg: string | null, logo: string | null) {
  ...
  // Lado direito branco (modelo base: x=5.0 w=5.04 h=5.76, não cobre toda altura)
  slide.addShape("rect", {
    x: 5.0, y: 0, w: 8.333, h: 7.5,
    fill: { color: COLORS.white }, line: { type: "none" },
  });

  if (logo) slide.addImage({ data: logo, x: 0.57, y: 0.42, w: 0.73, h: 0.2 });

  slide.addText("Status Mensal", {
    x: 0.59, y: 2.16, w: 4.41, h: 0.76,
    fontFace: FONT_HEADING, fontSize: 36, color: COLORS.white,
  });
  slide.addText(data.monthYear, {
    x: 0.59, y: 2.97, w: 7.91, h: 0.5,
    fontFace: "Inter Light", fontSize: 16, color: COLORS.white,
  });

  slide.addText(data.clientName.toUpperCase(), {
    x: 5.99, y: 2.33, w: 3.03, h: 1.07,
    fontFace: FONT_HEADING, fontSize: 36, color: COLORS.textDark,
...
function addDividerSlide(pptx: pptxgen, title: string, coverBg: string | null, logo: string | null) {
  ...
  if (parts.length === 2) {
    slide.addText(parts[0], {
      x: 0.6, y: 3.0, w: 7, h: 1.0,
      fontFace: FONT_HEADING, fontSize: 48, color: COLORS.white,
    });
...
function addBigNumbersTotalSlide(
  ...
  slide.addText("Total:", {
    ...
    fontFace: FONT_HEADING, fontSize: 11, bold: true, color: COLORS.textDark,
  });
  ...
  slide.addText(`${taxa}%`, {
    ...
    fontFace: "Inter Light", fontSize: 75, color: COLORS.cyan, align: "center",
  });
...
async function addResolvidosSlide(pptx: pptxgen, data: PresentationData, logo: string | null, bg: string | null) {
  const slide = pptx.addSlide();
  addSlideChrome(slide, logo, bg);

  // ── Esquerda: título grande + texto + plataformas embaixo ─────────────────
  slide.addText(data.resolvedTitle || "Mais de X agressores\nresolvidos em Janeiro.", {
    x: 0.4, y: 0.85, w: 7, h: 1.5,
    fontFace: FONT_HEADING, fontSize: 24, color: COLORS.primary,
  });

  slide.addText(
    "A Branddi atuou 24/7 em todas as plataformas\npara Detectar, Classificar e Remover.",
    { x: 0.4, y: 2.5, w: 7, h: 0.7, fontFace: FONT_BODY, fontSize: 11, color: COLORS.textDark }
  );

  // "Onde atuamos:" + ícones na parte inferior do slide
  slide.addText("Onde atuamos:", {
    x: 0.4, y: 5.85, w: 7, h: 0.35,
    fontFace: FONT_HEADING, fontSize: 11, color: COLORS.primary,
  });

  // Logos de plataformas reais na parte inferior
  const platformLogos = [
    { path: "/branding/platforms/google.png", label: "" },
    { path: "/branding/platforms/google-shopping.png", label: "" },
    { path: "/branding/platforms/google-play.png", label: "" },
    { path: "/branding/platforms/bing.png", label: "" },
    { path: "/branding/platforms/apple.png", label: "" },
    { path: "/branding/platforms/amazon-ads.png", label: "" },
  ];

  const platformImages = await Promise.all(platformLogos.map(p => loadAsset(p.path)));

  platformImages.forEach((imgData, i) => {
    const x = 0.5 + i * 1.1;
    const logoW = i === 5 ? 1.2 : 0.4;  // Amazon Ads é mais largo (texto)
    const logoH = 0.4;
    if (imgData) {
      slide.addImage({
        data: imgData,
        x: x + (i === 5 ? -0.1 : 0.3), y: 6.2, w: logoW, h: logoH,
...
function addSavingSlide(pptx: pptxgen, data: PresentationData, logo: string | null, bg: string | null) {
  ...
  // Saving (texto simples, sem box — como no modelo base)
  slideIs.addText("Saving", {
    x: 0.6, y: 1.78, w: 5.8, h: 0.3,
    fontFace: FONT_BODY, fontSize: 11, color: COLORS.textMuted, align: "center",
  });
  slideIs.addText(data.saving.savingValue || "—", {
    x: 0.6, y: 2.05, w: 5.8, h: 0.5,
    fontFace: FONT_HEADING, fontSize: 18, color: COLORS.textDark, align: "center",
  });

  // ROI (texto simples, sem box)
  slideIs.addText("ROI", {
    x: 6.93, y: 1.78, w: 5.8, h: 0.3,
    fontFace: FONT_BODY, fontSize: 11, color: COLORS.textMuted, align: "center",
  });
  slideIs.addText(data.saving.roiValue || "—", {
    x: 6.93, y: 2.05, w: 5.8, h: 0.5,
    fontFace: FONT_HEADING, fontSize: 18, color: COLORS.textDark, align: "center",
  });
  ...
  // Saving (texto simples, sem box)
  slideContains.addText("Saving", {
    x: 0.6, y: 1.78, w: 5.8, h: 0.3,
    fontFace: FONT_BODY, fontSize: 11, color: COLORS.textMuted, align: "center",
  });
  slideContains.addText(data.saving.savingValue || "—", {
    x: 0.6, y: 2.05, w: 5.8, h: 0.5,
    fontFace: FONT_HEADING, fontSize: 18, color: COLORS.textDark, align: "center",
  });
...
function addProximosPassosSlide(pptx: pptxgen, data: PresentationData, logo: string | null, bg: string | null) {
  ...
  enabledSteps.forEach((step, i) => {
    slide.addText(`${i + 1}.`, {
      ...
      fontFace: FONT_HEADING, fontSize: 12, bold: true, color: COLORS.cyan,
    });
    slide.addText(step.text, {
      x: 1.1, y: 1.7 + i * 0.7, w: 11.5, h: 0.5,
      fontFace: FONT_BODY, fontSize: 12, color: "666666",
    });
  });
}
...
function addEncerramentoSlide(pptx: pptxgen, coverBg: string | null, logo: string | null) {
  ...
  slide.addText("Você investe todos os dias para crescer.", {
    ...
    fontFace: "Archivo Light", fontSize: 40, color: COLORS.white,
  });
  ...
  slide.addText(
    "atendimento@branddi.com\n+55 11 92145-8912\nbranddi.com",
    {
      ...
      fontFace: "Inter Medium", fontSize: 10, color: COLORS.white,
    }
  );
}
...
export async function generatePresentationPpt(data: PresentationData, ativos?: Partial<SlidesAtivos>): Promise<void> {
  ...
  pptx.theme = { headFontFace: "Archivo SemiBold", bodyFontFace: "Inter" };

  const [logoBase64, coverBg, slideBg, logoWhite, closingBg, coverBgFull, dividerBg] = await Promise.all([
    loadAsset("/branding/logo-branddi.png"),
    loadAsset("/branding/cover-bg.png"),
    loadAsset("/branding/slide-bg.png"),
    loadAsset("/branding/logo-branddi-white.png"),
    loadAsset("/branding/closing-bg.png"),
    loadAsset("/branding/cover-bg-full.png"),
    loadAsset("/branding/divider-bg.png"),
  ]);

  // 1 - Capa (sempre)
  addCoverSlide(pptx, data, coverBgFull, logoWhite);
  // 2 - Divisor (sempre)
  addDividerSlide(pptx, "Brand Bidding", dividerBg, logoWhite);
  // 3
  ...
  // 21
  if (on("resolvidos")) await addResolvidosSlide(pptx, data, logoBase64, slideBg);
  // 22 - divisor campanha (só se algum slide de campanha estiver ativo)
  if (on("campanha") || on("saving")) addDividerSlide(pptx, "Resultados de campanha", dividerBg, logoWhite);
  // 23-27
  if (on("campanha")) {
